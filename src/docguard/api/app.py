...
_DASHBOARD_ASSET_VERSION = "markdown-report-v1"
...
def continue_task(task_id: str, background_tasks: BackgroundTasks):
    """Continue a disconnected artifact-backed attempt in its existing session."""
    try:
        task = store.get(task_id)
        ...
        if task.agent_backend not in {AgentBackend.OPENCLAW, AgentBackend.DSH}:
            raise ValueError(f"Task {task_id} does not use an artifact-backed backend")
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Task not found") from exc
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
